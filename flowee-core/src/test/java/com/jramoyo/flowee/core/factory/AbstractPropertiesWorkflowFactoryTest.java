...
public class AbstractPropertiesWorkflowFactoryTest {
	...
	public void testFetchActionNames() throws IOException {
		factory.init();
		Assert.assertEquals("Incorrect tasks", "task1",
				factory.fetchTaskNames("workflow1").get(0));
		Assert.assertEquals("Incorrect tasks", "task1",
				factory.fetchTaskNames("workflow2").get(0));
		Assert.assertEquals("Incorrect tasks", "task2",
				factory.fetchTaskNames("workflow2").get(1));
	}
	...
	public void testFetchConfiguration() throws IOException {
		...
		Assert.assertEquals("Incorrect value", "text == 'workflow1'",
				config.get("workflow1"));
		Assert.assertEquals("Incorrect value", "text == 'workflow2'",
				config.get("workflow2"));
	}

	private static class TestWorkflowFactory
			extends
			AbstractPropertiesWorkflowFactory<TestWorkflow, TestTask, String, WorkflowContext> {
		@Override
